      ...
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        ...
        <div className={`order-1 ${index % 2 === 0 ? 'lg:order-2' : 'lg:order-1'}`}>
          ...
          <div ref={tagsRef} className="flex flex-wrap gap-3 mb-12">
            {project.tags.map((tag, j) => <span key={`tag-${index}-${j}`} className="px-3 py-1 text-[9px] font-black uppercase text-white/40 border border-white/5 rounded-full bg-white/[0.02]">{t(`projects.tags.${tag}`)}</span>)}
          </div>
...
const Projects = () => {
  ...
  const projects = useMemo(() => [
    { title: t('projects.project1.title'), description: t('projects.project1.description'), image: PROJECT_IMAGES[0], tags: ['react', 'ts', 'go', 'arch'], links: { github: 'https://github.com/mattermost', demo: 'https://mattermost.com' } },
    { title: t('projects.project2.title'), description: t('projects.project2.description'), image: PROJECT_IMAGES[1], tags: ['electron', 'networking', 'ts'], links: { github: '#', demo: '#' } },
    { title: t('projects.project3.title'), description: t('projects.project3.description'), image: PROJECT_IMAGES[2], tags: ['uiohook', 'node', 'js'], links: { github: '#', demo: '#' } }
  ], [t]);
    ...
    <section ref={sectionRef} id="projects" className="relative py-48 bg-transparent overflow-hidden">
      <div ref={marqueeRef} className="relative mb-32 overflow-hidden py-8 border-y border-white/[0.04] select-none">
        <div data-marquee-track className="flex whitespace-nowrap">
          {[...Array(4)].map((_, i) => (
            <span key={`mq-${i}`} className="text-[6rem] md:text-[10rem] font-black text-white/[0.03] leading-none tracking-tight mx-4 font-display">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col">{projects.map((p, i) => <ProjectCard key={`pc-${i}`} project={p} index={i} totalProjects={projects.length} />)}</div>
      </div>
    </section>
  );
};
