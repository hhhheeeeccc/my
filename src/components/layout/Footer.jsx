  ...
  return (
    <footer className="py-20 bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-900 transition-colors duration-500 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        >
          <div className="mb-10">
            <span className="text-3xl font-black bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent tracking-tighter">M.Y.H.G</span>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-10">
            {footerLinks.map((link, idx) => (
              <motion.a key={idx} href={link.href} whileHover={{ scale: 1.1, y: -2 }} className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                {link.label}
              </motion.a>
            ))}
          </div>
          <div className="mt-16 pt-10 border-t border-slate-100 dark:border-slate-900 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex gap-8">
               {['privacy', 'terms'].map(key => (
                 <a key={key} href="#" className="text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">{t(`footer.${key}`)}</a>
               ))}
            </div>
          </div>
        </motion.div>
