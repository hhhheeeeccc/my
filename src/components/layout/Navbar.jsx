            ...
            <div className="flex justify-between items-center h-full">
              ...
              <div className="hidden md:flex gap-10 items-center">
                {navLinks.map((link, idx) => (
                  <Magnetic key={idx}>
                    <motion.a href={link.href} className="relative text-sm font-bold text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors group px-2 py-1">
                      {link.label}
              ...
              <div className="flex items-center gap-3">
                <Magnetic>
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => { const nl = isArabic ? 'en' : 'ar'; i18n.changeLanguage(nl).then(() => { document.documentElement.dir = nl === 'ar' ? 'rtl' : 'ltr'; document.documentElement.lang = nl; }); }} className="p-2.5 rounded-2xl hover:bg-slate-100/50 dark:hover:bg-slate-800/50 transition-colors text-slate-700 dark:text-slate-300">
                ...
                <Magnetic>
                  <motion.button whileHover={{ scale: 1.1, rotate: 15 }} whileTap={{ scale: 0.9 }} onClick={toggleTheme} className="p-2.5 rounded-2xl hover:bg-slate-100/50 dark:hover:bg-slate-800/50 transition-colors text-slate-700 dark:text-slate-300">
