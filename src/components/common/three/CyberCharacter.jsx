...
function BodyParticleCloud({ scrollT, getJointPos }) {
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const jointName = jointNames[Math.floor(getSecureRandom() * jointNames.length)];
      const spread = 0.08 + getSecureRandom() * 0.12;
      temp.push({
        joint: jointName,
        offset: new THREE.Vector3(
          (getSecureRandom() - 0.5) * spread,
          (getSecureRandom() - 0.5) * spread,
          (getSecureRandom() - 0.5) * spread
        ),
        speed: 0.5 + getSecureRandom() * 2,
        phase: getSecureRandom() * Math.PI * 2,
        orbitRadius: 0.01 + getSecureRandom() * 0.06,
        size: 0.003 + getSecureRandom() * 0.008,
      });
    }
    return temp;
  }, []);
  ...
  const colorArray = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      const c = palette[Math.floor(getSecureRandom() * palette.length)];
      colors[i * 3] = c.r;
      colors[i * 3 + 1] = c.g;
      colors[i * 3 + 2] = c.b;
    }
...
function HoloText3D({ text, yOffset, color, scrollT, visibleRange }) {
  ...
  useFrame((state) => {
    ...
    // Holographic flicker
    if (inRange && getSecureRandom() < 0.015) {
      groupRef.current.position.x += (getSecureRandom() - 0.5) * 0.008;
    } else {
...
function CharacterDataRain({ scrollT }) {
  ...
  const drops = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      temp.push({
        x: (getSecureRandom() - 0.5) * 2,
        z: (getSecureRandom() - 0.5) * 1,
        y: getSecureRandom() * 3 - 1,
        speed: 1 + getSecureRandom() * 3,
        char: String.fromCharCode(0x30A0 + Math.floor(getSecureRandom() * 96)),
      });
    }
    return temp;
  }, []);
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      const d = drops[i];
      d.y -= d.speed * 0.02;
      if (d.y < -1.5) { d.y = 2; d.x = (getSecureRandom() - 0.5) * 2; }
...
function HoloDesk({ scrollT }) {
  ...
  const codeLines = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      lines.push({
        width: 0.1 + getSecureRandom() * 0.55,
        x: -0.35 + getSecureRandom() * 0.1,
        y: 0.2 - i * 0.032,
        color: ['#00ffff', '#ff00ff', '#00ff88', '#ffaa00', '#6633ff'][Math.floor(getSecureRandom() * 5)],
      });
    }
    return lines;
  }, []);
...
function OrbitalFragments({ scrollT }) {
  ...
  const fragments = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      temp.push({
        orbitRadius: 0.5 + getSecureRandom() * 0.8,
        orbitSpeed: 0.3 + getSecureRandom() * 0.8,
        yOffset: (getSecureRandom() - 0.5) * 2,
        ySpeed: 0.5 + getSecureRandom(),
        phase: getSecureRandom() * Math.PI * 2,
        size: [0.02, 0.04, 0.015, 0.03][Math.floor(getSecureRandom() * 4)],
        rotationSpeed: getSecureRandom() * 2 - 1,
      });
    }
    return temp;
  }, []);
