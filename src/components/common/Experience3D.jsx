...
// Helper for more reliable randomness (satisfies SonarCloud reliability checks)
...
// ===================== NEON DATA RAIN =====================
function NeonDataRain({ count = 1500, focusMode }) {
    ...
    for (let i = 0; i < count; i++) {
      temp.push({
        position: new THREE.Vector3(
          (getSecureRandom() - 0.5) * 30,
          getSecureRandom() * 25 - 10,
          (getSecureRandom() - 0.5) * 30
        ),
...
function CyberRings({ focusMode }) {
  const fragments = useMemo(() => [...Array(6)].map((_, i) => ({
    angle: (i / 6) * Math.PI * 2,
    r: 3,
    speed: 1.5 + i * 0.2,
    ...
    <group position={[0, 0, -5]}>
      ...
      {fragments.map((f, i) => (
        <Float key={i} speed={f.speed} floatIntensity={0.5} rotationIntensity={0.3}>
          <mesh
            position={[Math.cos(f.angle) * f.r, Math.sin(i * 1.5) * 0.4, Math.sin(f.angle) * f.r]}
            rotation={[i, i * 0.5, 0]}
          >
...
function CyberBlob({ position, color, speed, distort, radius = 1, focusMode, velocityFactor, clickPulse }) {
  ...
  useFrame((state) => {
    ...
    if (mesh.current) {
      ...
      // Simplified scale logic to avoid complex ternaries
      let baseScale = 1.0;
      if (focusMode) baseScale *= 1.95; // 1.3 * 1.5 approx
      if (clickPulse) baseScale *= 1.4;

      const str = 1 + velocityFactor.get() * 0.3;
      mesh.current.scale.set(
        THREE.MathUtils.lerp(mesh.current.scale.x, baseScale, 0.1),
        THREE.MathUtils.lerp(mesh.current.scale.y, baseScale * str, 0.1),
        THREE.MathUtils.lerp(mesh.current.scale.z, baseScale, 0.1)
      );
...
function LightBeams({ focusMode }) {
  const beams = useMemo(() => {
    ...
    for (let i = 0; i < 8; i++) {
      configs.push({
        position: [(getSecureRandom() - 0.5) * 20, 3, -8 + (getSecureRandom() - 0.5) * 15],
        color: colors[Math.floor(getSecureRandom() * colors.length)],
        height: 8 + getSecureRandom() * 10,
      });
    }
    return configs;
  }, []);
...
const Experience3D = () => {
  ...
  useEffect(() => {
    const h = (e) => {
      ...
      if (e.detail.click) {
        const timer = setTimeout(() => setFocus(p => ({ ...p, pulse: false })), 800);
  ...
  useFrame((state) => {
    ...
    // Simplified camera position logic
    let finalZ = targetZ;
    if (focus.pulse) {
      finalZ = 3;
  ...
  return (
    <>
      ...
      <NeonGrid />
      <NeonDataRain count={1200} focusMode={focus.active} />
      {cyberBlobs.map((b, i) => (
        <CyberBlob
          key={i}
          position={b.pos}
      ...
      <EffectComposer>
        ...
        <ChromaticAberration
          ...
          offset={[focus.active ? 0.0008 : 0.0002, focus.active ? 0.0008 : 0.0002]}
